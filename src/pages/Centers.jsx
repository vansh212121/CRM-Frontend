...
//           <div>
//             <h1 className="text-2xl font-semibold text-foreground">Centers</h1>
//             <p className="mt-1 text-sm text-muted-foreground">
//               Manage registered medical centers
//             </p>
//           </div>
...
import { useState } from "react";
import { motion } from "framer-motion";
import { Plus, Download, Search, Filter, Building2, Users, Calendar, Activity } from "lucide-react";
import { DashboardLayout } from "@/components/layout/DashboardLayout";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import {
...
export default function Centers() {
  ...
  const stats = {
    ...
    totalPatients: centers.reduce((sum, center) => sum + (center.patientCount || 0), 0),
    totalAppointments: centers.reduce((sum, center) => sum + (center.appointmentCount || 0), 0),
  };
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="space-y-1">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-semibold text-foreground tracking-tight">Centers</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="rounded-lg border border-border/50">
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-semibold text-foreground mt-1">{stats.totalCenters}</p>
                    </div>
                    <div className="rounded-lg bg-accent/10 p-2 border border-accent/20">
            ...
            >
              <Card className="border-border/40 hover:border-border/60 transition-colors">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-muted-foreground/80">Active Centers</p>
                      <p className="text-2xl font-semibold text-success mt-1">{stats.activeCenters}</p>
                    </div>
            ...
            >
              <Card className="border-border/40 hover:border-border/60 transition-colors">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-muted-foreground/80">Total Patients</p>
                      <p className="text-2xl font-semibold text-foreground mt-1">{stats.totalPatients}</p>
                    </div>
                    <div className="rounded-lg bg-accent/10 p-2 border border-accent/20">
            ...
            >
              <Card className="border-border/40 hover:border-border/60 transition-colors">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-muted-foreground/80">Today's Appointments</p>
                      <p className="text-2xl font-semibold text-accent mt-1">{stats.totalAppointments}</p>
                    </div>
                    <div className="rounded-lg bg-accent/10 p-2 border border-accent/20">
            ...
            <div className="flex items-center gap-3">
              <Select value={districtFilter} onValueChange={setDistrictFilter}>
                ...
                <SelectContent className="rounded-lg border border-border/50">
                  ...
                  {districts.map((district) => (
                    <SelectItem key={district} value={district} className="text-sm rounded-lg">
                      {district}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="text-xs px-2 py-1 border-accent/30 text-accent bg-accent/10">
        ...
        >
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-foreground">All Centers</h3>
              <div className="flex items-center gap-2">
                <span className="text-xs text-muted-foreground/80">
                  Sorted by:
                </span>
